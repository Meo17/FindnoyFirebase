...
@section('title','List Inactive Field Officers')
@section('content')
                      ...
                      @foreach($field_officers as $key => $val)  
                        <?php $count++;?>
                          <tr>
                                ...
                                <td>{{$val['status']}} </td> 
                                <td>
                                  <a type="button" class="btn"style="color: #008c4d" data-book-id="{{$key}}"  id="btn_deactive" data-bs-toggle="modal" href="#verticalycentered3" d><i class="ri-user-add-fill" style="font-size: 25px;"></i></a>
                              </td>
                          </tr>
                      @endforeach
                    @endif
...
<script>
      $(document).ready(function () {
        $('#list_inactive_field_officers').addClass("active");
